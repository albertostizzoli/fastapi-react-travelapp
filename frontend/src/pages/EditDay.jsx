...
function EditDay() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => { // cerco il giorno con l'ID specificato
        ...
        res.data.forEach((travel) => { // ciclo su tutti i viaggi
          const d = travel.days.find((day) => day.id === parseInt(id)); // cerco il giorno con l'ID specificato
          if (d) dayFound = d; // se lo trovo lo salvo
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://127.0.0.1:8000/days/${id}`, day); // invio i dati aggiornati al backend
